  ...
  return (
    <Section>
      <div className="flex">
        ...
        <button
          className={`max-md:hidden bg-transparent py-5 px-12 rounded-20 flex gap-3 border-[#a259ff] border-2 items-center`}
        >
          <img
            src={rocket}
            alt="rocket"
      ...
      <div className="mt-14 max-xl:mt-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-[30px] max-md:gap-5">
        {creators.map((_, index) => (
          <Link
            to={`/artist/${index}`}
            key={index}
            className="bg-[#3b3b3b] rounded-[20px] p-5 flex xl:flex-col items-center justify-center"
          >
            <img
              src={avatar}
              alt="Avatar"
            ...
            <div className="xl:text-center xl:mt-5 max-xl:ml-5">
              <h5 className="text-[22px] text-white leading-[140%] font-semibold ">
                Animakid
              </h5>
              <p className="text-base leading-[140%] text-[#858584]">
                Total Sales:{" "}
                <span className="text-white font-[Space_Mono]">34.53 ETH</span>
              </p>
            </div>
          </Link>
        ))}
